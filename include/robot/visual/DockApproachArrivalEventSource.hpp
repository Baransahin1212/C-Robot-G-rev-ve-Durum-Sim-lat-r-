...
// Turns DockApproachController's own Docked state into exactly one
// HomeReached Event per docking (Phase 13X final-approach fix; Docked
// replaces the earlier Phase 13X "Arrived" state in Phase 13Y's precision
// reverse-docking redesign - see DockApproachController.hpp) - edge-
// triggered on DockApproachState::Docked going false -> true, mirroring
...
// (WaypointNavigator reaching computeDockStagingPoint(), Stage 1 only)
// must NEVER itself fire HomeReached anymore - that would end the mission
// the moment the robot reaches the staging point, before it has actually
// reversed in and made contact. WaypointArrivalEventSource remains fully
// wired for its other, unchanged role (frontier-target arrival during
// Haritalama, read directly via WaypointNavigator::state() - never via
// its own Event - see main3d.cpp's frontier-selection block).
//
// Holds only a const DockApproachController& - it never itself decides
// whether the robot has docked; it purely observes the state
