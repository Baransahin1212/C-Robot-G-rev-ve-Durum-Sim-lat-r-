...
It opens a real 1280x720 window titled **"Robot Simulator 3D"** showing a
small hard-coded demo world: a ground plane and grid, a two-wheel robot
model (body, two wheels, a red front heading marker), four box obstacles,
and a base/docking platform, viewed through a perspective camera you can
orbit with the mouse (drag to rotate, scroll to zoom - raylib's built-in
`CAMERA_FREE` mode) and a small on-screen HUD showing the robot's
position/heading and obstacle count. Close the window normally to exit.

**Phase 13M is visualization foundation only.** The robot is drawn
stationary at a fixed pose - it is **not** yet driven by
`RobotStateMachine`, `RobotRuntime`, or any FSM/sensor/command-script
logic, and `RobotSimulator3D` constructs none of those types. No physics,
collision detection, or navigation exists yet either. A later phase will
connect movement to the existing FSM/runtime; see
`docs/technical-decisions.md` (Phase 13M) for the full rationale and the
`robot_visual`/raylib dependency boundary.

**`RobotSimulator` never links raylib or `robot_visual`.** The two
executables are entirely independent - the CLI's build, dependencies, and
behavior are completely unaffected by the visual simulator's existence.
...
| `robot_visual_world` | `VirtualWorld`/`RobotPose`/`BoxObstacle`/`BasePlatform` (Phase 13M) - plain demo-scene data using this project's own `Vec3`, deliberately raylib-free. Depends only on `robot_domain` for the include directory. |
| `robot_visual` | `VisualRobot`/`Renderer3D` (Phase 13M), the raylib-based 3D drawing layer. Depends on `robot_visual_world` and `raylib`. This is the **only** point where this project depends on raylib - the dependency points inward, never the other way, and no robot-core/domain/hardware target links it. |
| `RobotSimulator3D` | The interactive 3D visual simulator executable (Phase 13M). Depends on `robot_visual` only - never links `robot_app`, and `RobotSimulator` never links it or raylib either. Constructs no `RobotStateMachine`/`RobotRuntime`/`RobotController` yet. |
