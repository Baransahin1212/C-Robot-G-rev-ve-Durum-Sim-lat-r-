...
## 3D visual simulator (Phase 13M/13N)
...
the robot's live FSM state/command, position, heading, and obstacle count.
`SPACE` pauses/resumes world movement only (camera and FSM stepping are
unaffected). Close the window normally to exit.
...
DemoCommandSource (visual-only, delivers ScenarioLoaded then StartMission)
  ...
  RobotRuntime
      |
      v
RobotStateMachine   ->   ScenarioLoaded: Idle -> Ready
      |                  StartMission:   Ready -> Moving
      v
 RobotController
      |
      v
VirtualRobotHardware   (IRobotHardware implementation, visual-simulator-only)
      |
      v
  VirtualWorld robot pose   (moves forward at a fixed ~1.0 unit/s while
      |                       the current command is MoveForward)
      v
   Renderer3D   ->   visible robot movement
```
...
per frame afterward, so the robot starts stationary, transitions through
`Ready` into `Moving` within the first couple of frames via the FSM's real
transition rules (never forced directly), and then visibly moves forward
in a straight line along its current heading, matching the direction the
red front marker points.

**Still deliberately simple - not full robotics simulation.** Movement is
constant-speed straight-line motion only (no turning, no differential
drive); there is no collision detection and obstacle boxes are not
treated as obstacles yet (no obstacle sensor exists in
`VirtualRobotHardware`); the robot's position is only clamped to the demo
world's ~10x10 bounds so it cannot drift away indefinitely; and
`ReturnToBase` is accepted as a command but not yet implemented as
navigation - it currently behaves like `Stopped`. See
`docs/technical-decisions.md` (Phase 13N) for the full rationale and the
...
| `robot_visual_world` | `VirtualWorld`/`RobotPose`/`BoxObstacle`/`BasePlatform` (Phase 13M) - plain demo-scene data using this project's own `Vec3`, deliberately raylib-free. `VirtualWorld::setRobotPosition()`/`setRobotHeading()` (Phase 13N) are its only mutation entry points. Depends only on `robot_domain` for the include directory. |
| `robot_visual` | `VisualRobot`/`Renderer3D` (Phase 13M), the raylib-based 3D drawing layer. Depends on `robot_visual_world` and `raylib`. This is the **only** point where this project depends on raylib - the dependency points inward, never the other way, and no robot-core/domain/hardware target links it. `Renderer3D` receives HUD state/command text as plain strings (Phase 13N) rather than depending on any FSM/hardware type. |
| `robot_visual_simulation` | `VirtualRobotHardware` (Phase 13N), the `IRobotHardware` implementation that is the visual simulator's FSM-driven actuator/sensor boundary - translates `RobotController` commands into `VirtualWorld` movement once per frame via `update()`. Depends on `robot_visual_world`, `robot_hardware`, `robot_controller`, `robot_runtime`, and `robot_domain` - the same real FSM/controller/runtime stack the CLI uses. Deliberately has no raylib dependency, and is a sibling of `robot_visual` (neither depends on the other) under `RobotSimulator3D`. |
| `RobotSimulator3D` | The interactive 3D visual simulator executable. Depends on `robot_visual` (rendering) and `robot_visual_simulation` (FSM-driven movement) - never links `robot_app`, and `RobotSimulator` never links either of them or raylib. As of Phase 13N, `main3d.cpp` constructs the real `RobotStateMachine`/`RobotController`/`RobotRuntime` (via `robot_visual_simulation`) plus a tiny visual-only `DemoCommandSource` to reach `Moving` through real FSM transitions. |
