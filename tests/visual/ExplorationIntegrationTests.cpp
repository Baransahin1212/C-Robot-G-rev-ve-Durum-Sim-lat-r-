...
{
    VirtualWorld world;
    // Keep only the one deterministic blocking obstacle directly ahead of
    // the robot's start pose (see VirtualWorld.cpp) so this test's
    // "obstacle becomes Occupied only after observation" assertion is
    // unambiguous - disable the other three scattered demo obstacles.
    for (std::size_t i = 0; i < world.obstacles().size(); ++i)
    {
        if (i != VirtualWorld::kBlockingObstacleIndex)
        {
            world.setObstacleEnabled(i, false);
        }
    }
    ...
    // any shortcut.
    const BoxObstacle& blockingObstacle = world.obstacles()[VirtualWorld::kBlockingObstacleIndex];
    int obstacleCol = -1;
    int obstacleRow = -1;
    ...
    // 9. Cells nowhere near the robot's path/sensors remain Unknown - the
    // far corner of the table, well away from the robot's short start-
    // area excursion.
    int farCol = -1;
    int farRow = -1;
    ASSERT_TRUE(h.explorationMap.worldToCell(robot::visual::Vec3{5.5F, 0.0F, -5.5F}, farCol, farRow));
...
TEST(ExplorationIntegrationTest, ExploringBeyondFormerHomeZoneContinuesMappingUntilUserRequestsReturnHome)
{
    ...
    ASSERT_GT(h.distanceToBase(), HomeZoneMonitor::kHomeZoneExitRadius); // 4
